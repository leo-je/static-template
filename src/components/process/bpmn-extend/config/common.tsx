...
import { FieldDefine } from '../../bpmn/components/dynamic-binder';
import { PropertiesMap, GroupProperties } from './index';
import SubList from '../../bpmn/components/sublist/SubList';
import { SubListState } from '../../bpmn/components/sublist/type';
 ...
 * 任务基本信息
 */
const TaskBaseProperties: PropertiesMap<FieldDefine> = {
  id: {
    component: ElInput,
    placeholder: '节点ID',
    vSlots: {
 ...
 * 任务基础信息配置组
 */
export const TaskBaseGroupProperties: GroupProperties = {
  name: '任务基础信息',
  icon: 'el-icon-info',
  properties: { ...TaskBaseProperties },
};

/**
 * 流程基本信息配置组
 */
const ProcessBaseProperties: PropertiesMap<FieldDefine> = {
  id: {
    component: ElInput,
    placeholder: '流程ID',
    vSlots: {
      prepend: (): JSX.Element => <div>流程ID</div>,
    },
    setValue(sourceObject: ModdleElement, key: string, value: string) {
      const isNotNull = value;
      const latestValue = value || ' ';
  ...
  name: {
    ...
    placeholder: '流程名称',
    vSlots: {
      prepend: (): JSX.Element => <div>流程名称</div>,
    },
  },
};
 ...
 * 流程基础信息
 */
export const ProcessBaseGroupProperties: GroupProperties = {
  name: '基础信息',
  icon: 'el-icon-info',
  properties: { ...ProcessBaseProperties },
};
...
export const ProcessDocumentGroupProperties: GroupProperties = {
  name: '流程文档',
  icon: 'el-icon-document',
  properties: {
    'documentation.text': {
 ...
 * 扩展属性组配置
 */
export const RoleGroupProperties: GroupProperties = {
  name: '审批角色',
  icon: 'el-icon-document-add',
  properties: {
    role: {
      ...
      placeholder: '审批角色',
      vSlots: {
        prepend: (): JSX.Element => <div>审批角色</div>,
      },
      rules: {
        role: [{ required: true, message: '审批角色不能为空' }],
      },
      getValue: (businessObject: ModdleElement): string => {
        ...
        let roleEl = businessObject?.extensionElements?.values
          ?.filter((item: RoleElement) => item.$type === 'activiti:Role')[0];
        console.log(roleEl)
        console.log(roleEl?.$attrs?.value)
        return roleEl?.$attrs?.value;
      },
      setValue(businessObject: ModdleElement, key: string, value: any): void {
        console.log(businessObject);
        console.log(key);
        console.log(value);
        const bpmnContext = BpmnStore;
        const moddle = bpmnContext.getModeler().get('moddle');
        const role = moddle.create(`activiti:Role`, { value });
        bpmnContext.updateExtensionElements('activiti:Role', role);
      },
    },
  },
};
